...
func TestGithubWebhookPasswordIsGenerated(t *testing.T) {
	dataDir, err := ioutil.TempDir("", "TestGithubWebhookPasswordIsGenerated")
	if err != nil {
		t.Fatalf("error creating data dir %v", err.Error())
	}
	...
	t.Run("github password is generated", func(t *testing.T) {
		payload, _ := ioutil.ReadFile(filepath.Join("fixtures", "hook_basic_push_payload.json"))
		req := httptest.NewRequest(echo.POST, "/api/"+apiVersion+"/pipeline/githook", bytes.NewBuffer(payload))
		req.Header.Set("Content-Type", "application/json")
		// Use https://www.freeformatter.com/hmac-generator.html#ad-output for example
		...
		// Expected failure because secret will not match with fixture
		...
		v.LoadSecrets()
		_, err := v.Get("GITHUB_WEBHOOK_SECRET")
		if err != nil {
			t.Fatal("did not expect error to happen. got: ", err.Error())
		}
	})
