...
func init() {
	...
	fs.StringVar(&gaia.Cfg.WorkerHostURL, "worker-host-url", "http://192.168.1.123:8080", "The host url of an Gaia primary instance to connect to. Only used in worker mode or for docker runs")
	fs.StringVar(&gaia.Cfg.WorkerGRPCHostURL, "worker-grpc-host-url", "192.168.1.123:8989", "The host url of an Gaia primary instance gRPC interface used for worker connection. Only used in worker mode or for docker runs")
